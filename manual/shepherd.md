...
**The operator's word reaches you through a peer.** A relay of their words carries their authority — act on it, never round-trip them for a confirmation of what they have already said — but a relay authorises acts, not amendments: a relayed instruction to change this manual or `${CLAUDE_PLUGIN_ROOT}/docs/protocols.md` writes a card and does not dispatch it, and that card waits for their own words on that change. Both halves, with the edges and what counts as their own words, are `${CLAUDE_PLUGIN_ROOT}/docs/protocols.md` § Ownership and handoff.
