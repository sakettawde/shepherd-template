...
- Workers end every pause and final message with `SHEPHERD: done|blocked|failed|working — <one-liner>`, on a line of its own. **`blocked` means the worker needs your input to continue** — a design approval, an answer, a ruling, a permission — and reaches you within seconds through the status-file watcher; `working` means the worker continues on its own next turn and is never terminal, so it waits for the heartbeat. `templates/task-card.md` `### Status protocol` is the canonical wording every Brief carries. The Stop hook records the **last such line** in the turn, so a worker that merely writes *about* a claim inside a paragraph no longer records one.
...
- **Context check — you measure yourself, you roll yourself over.** `scripts/context-rollover.sh decide` reads the status line Claude Code renders in your own pane (`~/.claude/statusline.py`, user-global) and answers `ok | hold | rollover | unknown`. Run it at every point you are already awake: session-start step 10, the end of every monitor wake, retro step 7 before dispatching the next card, and after any triage that ends with nothing dispatched. `rollover` → run adapter R10 now, in that same turn — one command, `scripts/context-rollover.sh rollover`, then end the turn. It needs no argument: the recovery line it types into the fresh session defaults to `/wake`. Thresholds are **absolute tokens OR percent, whichever fires first** (on a 1M window a comfortable percent is already an expensive, degraded turn): idle (no active card you own) at **≥200k tokens or ≥60 %**; regardless at **≥350k tokens or ≥85 %** — recovery re-arms watchers from the cards. `hold` → say one line to the operator ("ctx NN %, rolling over at next close-out") and roll over at the next close-out. `unknown` → the status line is not visible in the pane; fall back to `context-rollover.sh meter <session>` (approximate) and say so. Never roll over mid-verification or while holding `dispatch`/a card lock.

  **A rollover either completes or reports — never neither.** The foreground call is **read-only**: it refuses before anything is armed (a `blocked` pane, no session id), logs, and exits. Every keystroke — `Escape`, the bash-mode read-back, `/clear`, the recovery prompt — comes from the **detached watchdog**, which waits for the foreground pid to exit first. That order is the fix for the 2026-08-27 stalls: an `Escape` sent to your own pane from the foreground reaches Claude Code as *interrupt the running tool*, and the running tool is the call carrying the script (R10, `docs/specs/context-rollover-design.md` §8). The watchdog gates on the pane's **Claude session id changing**, never on `agent_status: idle` — a shepherd pane can never report `idle`, because its own watcher shells outrank the idle rule. Every poll and every verdict lands in `${SHEPHERD_ROLLOVER_LOG:-~/.claude/shepherd-rollover.log}`, and every give-up raises a toast naming the line to type by hand. If the command exits non-zero, **you did not roll over** — say so to the operator and do not act as if you had. A `0` means it armed, not that it recovered: the log tail at wake (step 10) is what confirms that.
- **Session start → the `wake` skill.** Every fresh session runs it as its first act:
  ...
  Four rules govern the run, and they live here:

  1. **Order is load-bearing.** Each step reads state an earlier step changes. Run 1 to
     10 and skip none.
  2. **Steps 5 to 9 are owner-filtered.** You act on the cards whose `owner:` is you
     (§2 rule 10, §4a), and on no others.
  3. **Repair only the unambiguous direction** — you own the card and no live instance
     holds the lock. Report every other mismatch instead of repairing it.
  4. **Report every sweep line kind to the operator.** `SWEPT` is routine — say how
     many. Everything else gets named.
