...
4. **Register hooks user-globally** — merge into `~/.claude/settings.json` (create it as `{}` if absent) using the python3 snippet below: a `Stop` entry (matcher `*`) running `bash '<shepherd-root>/hooks/worker-stop.sh'`, a `Notification` entry (matcher `permission_prompt|idle_prompt|elicitation_dialog|agent_needs_input|agent_completed`) running `bash '<shepherd-root>/hooks/worker-notify.sh'`, and a `PreToolUse` entry (matcher `Bash`) running `bash '<shepherd-root>/hooks/worker-git-guardrail.sh'` (blocks destructive git in worker sessions), all `timeout: 10`. The snippet first removes any existing hook whose command ends in the same script name (that is the re-run/new-PC path), then appends the fresh entries. All unrelated settings and hooks are preserved verbatim.

   The `Notification` matcher is **exact strings joined by `|`** — regex is not supported for this event — and the nine values it may take are `permission_prompt`, `idle_prompt`, `auth_success`, `elicitation_dialog`, `elicitation_url_dialog`, `elicitation_complete`, `elicitation_response`, `agent_needs_input`, `agent_completed` (Notification reference, <https://code.claude.com/docs/en/hooks>, read 2026-08-23). The last two need **Claude Code v2.1.198 or later** and fire for *background sessions* only, while the agent view is open (matcher reference, <https://code.claude.com/docs/en/hooks-guide>) — so on a herdr-pane worker they are expected to fire **zero** times. They are registered anyway: the listener costs nothing, it is already correct if a worker ever runs as a background session, and a non-zero count is the measurement that lets adapter R5 adopt `agent_completed` as a wake signal (R5's "adopting `agent_completed`" note holds the confirm step). If the operator's Claude Code is older than 2.1.198, drop the two names from the matcher — an unknown matcher value silently never fires.
   ...
   install("Notification", "permission_prompt|idle_prompt|elicitation_dialog|agent_needs_input|agent_completed", "worker-notify.sh")
...
7. **Confirm** — one line: operator name, code dir, worker cap, hook file path, registry rows, and the canonical launch command from CLAUDE.md §1 **with their own `SHEPHERD_ID` substituted in all three places** (the env var, `-n` and `--remote-control` carry the same id). Remind the operator that hooks fire only in *worker* sessions (they exit instantly without `SHEPHERD_TASK_ID`).
