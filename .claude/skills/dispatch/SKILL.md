...
2. **Launch** — adapter R3 with the card's tier: standard → `--model opus --effort high`; heavy → `--model opus --effort xhigh`. **A different model only when the operator specifically asked for it, and then only at `--effort high`.** Env vars in the launch line: `SHEPHERD_TASK_ID=T-NNNN`, `SHEPHERD_STATUS_FILE=<shepherd-root>/ledger/status/T-NNNN.jsonl`. Session name: `-n worker-T-NNNN`, the card's own id — it is what a peer's `ListAgents` shows, so one listing tells workers from shepherd instances and names the card each worker runs ([CLI reference](https://code.claude.com/docs/en/cli-reference), read 2026-08-28; adapter R3 holds the whole line and the collision caveat).
