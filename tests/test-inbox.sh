#!/usr/bin/env bash
# Probe for shepherd-inbox — the shepherd side of the Linear agent inbox.
#
...
. "$HERE/harness.sh"
sandbox
SCRIPT="$HERE/../bin/shepherd-inbox"
...
# not change the public CLI: `shepherd-inbox pending` still prints a bare integer.
...
# === T-0237: the conversation verbs (spec 2026-09-07 §5) ====================
# T-0236 lands the Worker side in parallel, so every shape below is the spec's,
# served by the stub; the deployed Worker still lacks author, operator_ids and
# the signal passthrough while this runs.
route sessions_s1_activity 200 '{"ok":true,"activityId":"a2"}'
body_of() { last_request | field body; }
jf() { python3 -c 'import json,sys; d=json.load(sys.stdin); print(eval("d"+sys.argv[1]))' "$1"; }

# --- action: the Worker's own shape, so its own verb ------------------------
# `activity <sid> action <body>` built {type, body}, which the Worker answers
# 400 "invalid content" - an action carries action/parameter/result instead.
assert_ok "action exits 0" "$SCRIPT" action s1 "bash scripts/tests/run.sh" "exit 0" "ALL TESTS PASSED"
sent=$(body_of)
assert_eq "action sends type action"     "$(printf '%s' "$sent" | jf '["type"]')"      "action"
assert_eq "action sends the action"      "$(printf '%s' "$sent" | jf '["action"]')"    "bash scripts/tests/run.sh"
assert_eq "action sends the parameter"   "$(printf '%s' "$sent" | jf '["parameter"]')" "exit 0"
assert_eq "action sends the result"      "$(printf '%s' "$sent" | jf '["result"]')"    "ALL TESTS PASSED"
assert_eq "action sends no body key"     "$(printf '%s' "$sent" | jf '.get("body","absent")')" "absent"
assert_eq "action posts to the activity endpoint" "$(last_request | field path)" "/sessions/s1/activity"
"$SCRIPT" action s1 "ran" "x" >/dev/null 2>&1
assert_eq "action without a result omits the key rather than sending an empty one" \
  "$(body_of | jf '.get("result","absent")')" "absent"
: > "$LOG"
"$SCRIPT" action s1 only-one >/dev/null 2>&1;      assert_eq "action with one argument is usage" "$?" "2"
"$SCRIPT" action s1 a b c d >/dev/null 2>&1;       assert_eq "action with a fifth argument is usage" "$?" "2"
"$SCRIPT" activity s1 action "ran x" >/dev/null 2>&1; assert_eq "activity refuses the action type: it has its own verb" "$?" "2"
assert_eq "…and none of them sent anything" "$(wc -l < "$LOG")" "0"

# --- ephemeral and select ----------------------------------------------------
assert_ok "activity --ephemeral exits 0" "$SCRIPT" activity s1 thought "a reader is on it" --ephemeral
sent=$(body_of)
assert_eq "--ephemeral sends ephemeral true" "$(printf '%s' "$sent" | jf '["ephemeral"]')" "True"
assert_eq "--ephemeral keeps the body"       "$(printf '%s' "$sent" | jf '["body"]')" "a reader is on it"
"$SCRIPT" activity s1 thought "plain" >/dev/null 2>&1
assert_eq "without --ephemeral no ephemeral key is sent" "$(body_of | jf '.get("ephemeral","absent")')" "absent"
assert_ok "--ephemeral is accepted ahead of the positionals too" "$SCRIPT" activity --ephemeral s1 thought "x"
assert_eq "…and still sends it" "$(body_of | jf '["ephemeral"]')" "True"
assert_ok "-- ends the options, so a body starting with -- can be sent" "$SCRIPT" activity s1 thought -- "--verbose was the flag"
assert_eq "…and it arrives whole" "$(body_of | jf '["body"]')" "--verbose was the flag"

assert_ok "activity --select exits 0" "$SCRIPT" activity s1 elicitation "Fix now or leave it?" --select "fix now=fix|leave it=leave"
sent=$(body_of)
assert_eq "--select sends signal select" "$(printf '%s' "$sent" | jf '["signal"]')" "select"
assert_eq "--select sends the options as label/value pairs, in order" \
  "$(printf '%s' "$sent" | jf '["signalMetadata"]["options"]')" \
  "[{'label': 'fix now', 'value': 'fix'}, {'label': 'leave it', 'value': 'leave'}]"
assert_eq "--select keeps the body" "$(printf '%s' "$sent" | jf '["body"]')" "Fix now or leave it?"
"$SCRIPT" activity s1 elicitation "x" --select "Preview=https://x.dev/?v=1" >/dev/null 2>&1
assert_eq "a value keeps its own = : only the first one splits" \
  "$(body_of | jf '["signalMetadata"]["options"][0]["value"]')" "https://x.dev/?v=1"
"$SCRIPT" activity s1 elicitation "plain" >/dev/null 2>&1
assert_eq "without --select no signal is sent" "$(body_of | jf '.get("signal","absent")')" "absent"
assert_eq "…and no signalMetadata"             "$(body_of | jf '.get("signalMetadata","absent")')" "absent"
# select "presents a list of options for the user to choose from as part of an
# elicitation activity" (linear.app/developers/agent-signals, read 2026-09-07):
# on any other type it is usage here, not a 400 from Linear later.
for type in thought response error; do
  : > "$LOG"
  "$SCRIPT" activity s1 "$type" "x" --select "a=1" >/dev/null 2>&1; rc=$?
  assert_eq "--select on a $type is usage" "$rc" "2"
  assert_eq "…and the $type was not sent" "$(wc -l < "$LOG")" "0"
done
for bad in "a=1|=2" "a=1|b" "=1" "a=" "" "|"; do
  : > "$LOG"
  "$SCRIPT" activity s1 elicitation "x" --select "$bad" >/dev/null 2>&1; rc=$?
  assert_eq "--select [$bad] is usage" "$rc" "2"
  assert_eq "…and [$bad] sent nothing" "$(wc -l < "$LOG")" "0"
done
"$SCRIPT" activity s1 thought >/dev/null 2>&1;         assert_eq "activity with no body is still usage" "$?" "2"
"$SCRIPT" activity s1 thought x --mumble >/dev/null 2>&1; assert_eq "an unknown option is usage" "$?" "2"

# --- urls: the external-URL endpoint --------------------------------------------
route sessions_s1_external-url 200 '{"ok":true}'
assert_ok "urls exits 0" "$SCRIPT" urls s1 "Branch=https://github.com/o/r/tree/task/T-1" "PR=https://github.com/o/r/pull/7?x=1"
assert_eq "urls posts to the external-url endpoint" "$(last_request | field path)" "/sessions/s1/external-url"
assert_eq "urls uses POST" "$(last_request | field method)" "POST"
assert_eq "urls sends {urls:[{label,url}]}, in order, the url keeping its own =" \
  "$(body_of | jf '["urls"]')" \
  "[{'label': 'Branch', 'url': 'https://github.com/o/r/tree/task/T-1'}, {'label': 'PR', 'url': 'https://github.com/o/r/pull/7?x=1'}]"
: > "$LOG"
"$SCRIPT" urls s1 >/dev/null 2>&1;             assert_eq "urls with no pair is usage" "$?" "2"
"$SCRIPT" urls s1 "no-equals" >/dev/null 2>&1; assert_eq "a pair without = is usage" "$?" "2"
"$SCRIPT" urls s1 "=https://x" >/dev/null 2>&1; assert_eq "an empty label is usage" "$?" "2"
"$SCRIPT" urls s1 "Preview=" >/dev/null 2>&1;   assert_eq "an empty url is usage" "$?" "2"
"$SCRIPT" urls s1 "A=https://a" "bad" >/dev/null 2>&1; assert_eq "one bad pair refuses the whole call" "$?" "2"
"$SCRIPT" urls s1 " =https://x" >/dev/null 2>&1;   assert_eq "a whitespace-only label is usage" "$?" "2"
"$SCRIPT" urls s1 "Preview= " >/dev/null 2>&1;     assert_eq "a whitespace-only url is usage" "$?" "2"
"$SCRIPT" urls s1 "Pre view=https://x" >/dev/null 2>&1; assert_eq "a label with a space is usage: it is a link label, not a sentence" "$?" "2"
assert_eq "…and none of them sent anything" "$(wc -l < "$LOG")" "0"

# --- list carries the author, and survives a Worker that has none -------------
route inbox 200 '{"events":[{"id":8,"kind":"prompted","session_id":"s1","author":{"id":"u1","name":"S","operator":true}}],"cursor":8}'
out=$("$SCRIPT" list)
assert_eq "list prints the author the Worker sends" \
  "$(printf '%s' "$out" | jf '["events"][0]["author"]')" "{'id': 'u1', 'name': 'S', 'operator': True}"
route inbox 200 '{"events":[{"id":9,"kind":"created","session_id":"s2"}],"cursor":9}'
out=$("$SCRIPT" list); rc=$?
assert_eq "list exits 0 against a Worker that predates author" "$rc" "0"
assert_eq "…and prints the event unchanged, no author invented" \
  "$(printf '%s' "$out" | jf '["events"][0]')" "{'id': 9, 'kind': 'created', 'session_id': 's2'}"

# --- operators: /health's operator_ids -------------------------------------------
route health 200 '{"status":"ok","shepherd_id":"shepherd-fixture","operator_ids":["u1","u2"]}'
out=$("$SCRIPT" operators); rc=$?
assert_eq "operators exits 0" "$rc" "0"
assert_eq "operators prints the ids one per line" "$out" "$(printf 'u1\nu2')"
assert_eq "operators does not authenticate" "$(last_request | field auth)" ""
route health 200 '{"status":"ok","shepherd_id":"shepherd-fixture","operator_ids":[]}'
out=$("$SCRIPT" operators); rc=$?
assert_eq "an empty list exits 0" "$rc" "0"
assert_eq "…and prints nothing" "$out" ""
route health 200 '{"status":"ok","shepherd_id":"shepherd-fixture"}'
out=$("$SCRIPT" operators); rc=$?
assert_eq "a Worker that predates operator_ids exits 0" "$rc" "0"
assert_eq "…and prints nothing: the same old world" "$out" ""
route health 200 '{"status":"ok","shepherd_id":"shepherd-other","operator_ids":["u9"]}'
assert_eq "operators reads another instance's Worker too: a read, not a gate" "$("$SCRIPT" operators 2>/dev/null)" "u9"
route health 503 '{"error":"unavailable"}'
"$SCRIPT" operators >/dev/null 2>&1; assert_eq "an unreachable /health exits 1" "$?" "1"
route health 200 'not json'
"$SCRIPT" operators >/dev/null 2>&1; assert_eq "a /health that is not JSON exits 1" "$?" "1"
route health 200 '{"status":"ok","shepherd_id":"shepherd-fixture","operator_ids":"u1"}'
"$SCRIPT" operators >/dev/null 2>&1; assert_eq "an operator_ids that is not a list exits 1" "$?" "1"
route health 200 '{"status":"ok","shepherd_id":"shepherd-fixture"}'

# --- the new verbs keep the token out of every stream ---------------------------
route sessions_s1_activity 401 '{"error":"unauthorized"}'
route sessions_s1_external-url 401 '{"error":"unauthorized"}'
noise=$("$SCRIPT" action s1 a b 2>&1; "$SCRIPT" urls s1 A=https://a 2>&1; "$SCRIPT" operators 2>&1
        "$SCRIPT" activity s1 elicitation x --select "a=1" --ephemeral 2>&1; "$SCRIPT" list 2>&1)
case $noise in *"$TOKEN"*) fail "no new verb prints the token either" "token found in output" ;;
               *) ok "no new verb prints the token either" ;; esac
route sessions_s1_activity 200 '{"ok":true,"activityId":"a3"}'

# --- log: the ledger/inbox.log writer (spec 2026-09-07 §7) ---------------------
# The drain writes the first shape after the first word posts; retro appends
# the second when a carded event's response posts; shepherd-metrics reads
# both. The writer is positional in the line's own order and validates what the
# parser would otherwise have to guess at.
export TZ=UTC
INBOX_LOG="$SHEPHERD_ROOT/ledger/inbox.log"
export SHEPHERD_NOW_OVERRIDE="2026-09-07T10:05:00+00:00"
received_ms=$(( $(date -u -d 2026-09-07T09:58:00Z +%s) * 1000 ))
out=$("$SCRIPT" log 2026-09-07T10:00:00+00:00 41 sess-a prompted ask operator answered "$received_ms"); rc=$?
assert_eq "log exits 0" "$rc" "0"
line="2026-09-07T10:00:00+00:00 41 sess-a prompted ask operator answered 2026-09-07T09:58:00+00:00 2026-09-07T10:05:00+00:00"
assert_eq "log writes the line: received-at from the Worker's epoch ms as ISO, first-word-at defaulting to now" \
  "$(tail -n1 "$INBOX_LOG")" "$line"
assert_eq "…and prints it, for the card's Log" "$out" "$line"
"$SCRIPT" log 2026-09-07T10:00:00+00:00 42 none created build member carded:T-0301 2026-09-07T09:59:00+00:00 2026-09-07T10:02:00+00:00 >/dev/null
assert_eq "log takes an explicit first-word-at" "$(tail -n1 "$INBOX_LOG")" \
  "2026-09-07T10:00:00+00:00 42 none created build member carded:T-0301 2026-09-07T09:59:00+00:00 2026-09-07T10:02:00+00:00"
"$SCRIPT" log 2026-09-07T15:30:00+05:30 44 s2 prompted status unknown refused "$(date -u -d 2026-09-07T09:59:00Z +%s)" >/dev/null
assert_eq "log normalises every offset to one clock, epoch seconds included" "$(tail -n1 "$INBOX_LOG")" \
  "2026-09-07T10:00:00+00:00 44 s2 prompted status unknown refused 2026-09-07T09:59:00+00:00 2026-09-07T10:05:00+00:00"
out=$("$SCRIPT" log answered 42)
assert_eq "log answered defaults answer-at to now" "$(tail -n1 "$INBOX_LOG")" "answered 42 2026-09-07T10:05:00+00:00"
assert_eq "…and prints that line too" "$out" "answered 42 2026-09-07T10:05:00+00:00"
"$SCRIPT" log answered 42 2026-09-07T12:00:00Z >/dev/null
assert_eq "log answered takes an explicit answer-at, Z read as UTC" "$(tail -n1 "$INBOX_LOG")" "answered 42 2026-09-07T12:00:00+00:00"
assert_eq "log appends: every line so far is still there" "$(wc -l < "$INBOX_LOG")" "5"
before=$(wc -l < "$INBOX_LOG")
T0=2026-09-07T10:00:00+00:00; R0=2026-09-07T09:59:00+00:00
for args in "answered" "answered 42 not-a-time" "answered 42 $T0 extra" \
  "$T0 43 s prompted ask operator" \
  "$T0 43 s prompted ask operator answered" \
  "$T0 43 s prompted ask operator mumble $R0" \
  "$T0 43 s prompted ask operator carded $R0" \
  "$T0 43 s prompted ask operator carded:T-1 $R0" \
  "$T0 43 s prompted ask boss answered $R0" \
  "yesterday 43 s prompted ask operator answered $R0" \
  "$T0 43 s prompted ask operator answered $R0 soon" \
  "$T0 43 s prompted ask operator answered $R0 $T0 extra"; do
  # shellcheck disable=SC2086
  "$SCRIPT" log $args >/dev/null 2>&1; rc=$?
  assert_eq "log refuses [$args] with 2" "$rc" "2"
done
"$SCRIPT" log "$T0" "43 44" s prompted ask operator answered "$R0" >/dev/null 2>&1; rc=$?
assert_eq "log refuses whitespace inside a column" "$rc" "2"
"$SCRIPT" log "$T0" 43 "" prompted ask operator answered "$R0" >/dev/null 2>&1; rc=$?
assert_eq "log refuses an empty column" "$rc" "2"
err=$("$SCRIPT" log answered 42 "²" 2>&1); rc=$?
assert_eq "log refuses a non-ASCII digit with 2" "$rc" "2"
case $err in *Traceback*) fail "…and without a traceback" "got [$err]" ;; *) ok "…and without a traceback" ;; esac
assert_eq "…and none of the refusals wrote a line" "$(wc -l < "$INBOX_LOG")" "$before"
SHEPHERD_INBOX_ENV="$SHEPHERD_ROOT/absent.env" "$SCRIPT" log answered 42 2026-09-07T12:00:00Z >/dev/null 2>&1; rc=$?
assert_eq "log needs no Worker config" "$rc" "0"
