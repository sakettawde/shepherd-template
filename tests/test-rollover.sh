#!/usr/bin/env bash
# Probe for shepherd-rollover's rollover/watch path — the sequence that
...
# word Claude Code resolves deterministically: the repo's own `skills/`
# entries are user-invocable as `/<name>`, so `/shepherd:wake` reaches the wake skill
...
# shepherd turn actually types is `shepherd-rollover rollover` and nothing else.
MSG="/shepherd:wake"
...
# === F13: the deprecation shim at the old path is GONE (T-0219) =============
# T-0185 left `scripts/self-recycle.sh` as a one-release forwarding shim so an
# instance mid-upgrade kept working. Every checkout is on the new name now, and a
# shim that outlives its purpose is one more path a permission classifier can read
# as "recycle". Its absence is asserted so nobody restores it by habit.
assert_nofile "the self-recycle.sh shim is gone" "$HERE/../bin/self-recycle.sh"
...
# so `shepherd-rollover rollover` is the whole invocation and every caller
# gets the same recovery line for free.
assert_ok "the script carries a default recovery message" \
  grep -q 'ROLLOVER_MSG:-/shepherd:wake' "$SCRIPT"
assert_ok "the default is one namespaced skill, slash-prefixed, no prose" \
  bash -c 'grep -oE "ROLLOVER_MSG:-[^}]*" "$1" | grep -qE "^ROLLOVER_MSG:-/[a-z]+:[a-z]+$"' _ "$SCRIPT"

scene SID-OLD working "❯" now
HANDOFF_OVERRIDE=30 run rollover --pane w1:p1
assert_eq "rollover with no message still arms" "$?" "0"
assert_ok "the default message is what got armed" grep -q 'msg=/shepherd:wake' "$LOG"
...
scene SID-OLD working "❯" now
run watch w1:p1 SID-OLD "/shepherd:wake"
assert_eq "the default message verifies end to end" "$?" "0"
assert_ok "the fresh session is prompted with /shepherd:wake" \
  grep -q 'agent prompt w1:p1 /shepherd:wake' "$CALLS"
...
# is gone, `/shepherd:wake` is an unknown slash command and the fresh session stands up
