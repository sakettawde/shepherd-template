...
# The clock is stubbed (T-0217 §8): two fixed instants, no sleep, exact
# assertions. Before this, the test snapshotted last_seen, slept 1 s and
# asserted "changed" - on a second-granularity clock under WSL2 clock
# corrections it failed ~2 in 6 full-suite runs (T-0128 item 2).
export SHEPHERD_NOW_OVERRIDE=2026-01-01T10:00:00+00:00
...
assert_eq   "registration stamps started from the one clock" "$(reg_field "$S/shepherd-1.json" started)" "2026-01-01T10:00:00+00:00"

# re-acquire from the SAME pane+session is idempotent (a restart of the script)
export SHEPHERD_NOW_OVERRIDE=2026-01-01T10:00:05+00:00
...
assert_eq "idempotent re-acquire preserves the original started timestamp" \
  "$(reg_field "$S/shepherd-1.json" started)" "2026-01-01T10:00:00+00:00"
assert_eq "idempotent re-acquire refreshes last_seen" \
  "$(reg_field "$S/shepherd-1.json" last_seen)" "2026-01-01T10:00:05+00:00"
...
assert_fail "rejects an uppercase suffix"   bash "$I" acquire 'shepherd-Collie'
assert_fail "rejects a trailing hyphen"     bash "$I" acquire 'shepherd-collie-'
assert_fail "rejects a doubled hyphen"      bash "$I" acquire 'shepherd--collie'
assert_fail "rejects a dotted suffix"       bash "$I" acquire 'shepherd-col.lie'
...
# still sees it.
for fun in collie kelpie huntaway; do
...
# shepherd-lock's own "MALFORMED - left in place" convention). Both an empty
...
export SHEPHERD_NOW_OVERRIDE=2026-01-01T11:00:00+00:00
assert_ok "touch by the owning instance succeeds" bash "$I" touch shepherd-2
assert_eq "touch refreshes last_seen from the one clock" "$(reg_field "$S/shepherd-2.json" last_seen)" "2026-01-01T11:00:00+00:00"
assert_eq "touch preserves started" "$(reg_field "$S/shepherd-2.json" started)" "$started_before"
unset SHEPHERD_NOW_OVERRIDE
...
# race_copy <dest> — a private copy of shepherd-identity + shepherd-lock + the
...
race_copy() {
  local dest=$1
  # Mirror the PLUGIN's shape, not a flat directory: every bin/ command sources
  # ../lib/shepherd-common.sh and calls its siblings by "$HERE/shepherd-<name>",
  # so the copy only behaves like the real thing when bin/ and lib/ are siblings.
  mkdir -p "$dest/bin" "$dest/lib"
  cp "$HERE/../bin/shepherd-identity" "$dest/bin/shepherd-identity"
  cp "$HERE/../bin/shepherd-lock" "$dest/bin/shepherd-lock"
...
out=$(bash "$race/bin/shepherd-identity" acquire shepherd-9 2>&1 >/dev/null); rc=$?
...
# may free an id, the same safe polarity shepherd-lock's sweep uses.
...
# and the only cleaner (shepherd-lock sweep) runs at session-start step 3, AFTER
...
out=$(bash "$race2/bin/shepherd-identity" acquire shepherd-10 2>&1 >/dev/null); rc=$?
...
# --- T-0217 §2: every identity verdict is one events line -------------------
E="$SHEPHERD_ROOT/ledger/events.log"
rm -f "$E"
export HERDR_PANE_ID=w6:p20
export SHEPHERD_PANE_SESSION_OVERRIDE=sess-20
export SHEPHERD_LIVENESS_OVERRIDE="w6:p20:sess-20 w6:p21:sess-21"
unset SHEPHERD_LIVENESS_UNKNOWN
bash "$I" acquire shepherd-20 >/dev/null
assert_eq "a fresh acquire logs IDENTITY under the id" \
  "$(awk '$3 == "identity" {print $2, $4, $5}' "$E" | tail -1)" "shepherd-20 shepherd-20 IDENTITY"
bash "$I" acquire shepherd-20 >/dev/null
# Verdict plus the two detail words only this branch writes. The preceding
# line is already IDENTITY, so a verdict-only compare passes even when the
# "already mine" branch logs nothing at all - the detail is what makes the
# assertion notice.
assert_eq "an idempotent re-acquire logs IDENTITY too" \
  "$(awk '$3 == "identity" {print $5, $(NF-1), $NF}' "$E" | tail -1)" "IDENTITY already mine"
bash "$I" touch shepherd-20 >/dev/null
assert_eq "touch logs TOUCHED under verb identity-touch" \
  "$(awk '$3 == "identity-touch" {print $2, $4, $5}' "$E" | tail -1)" "shepherd-20 shepherd-20 TOUCHED"
export HERDR_PANE_ID=w6:p21
export SHEPHERD_PANE_SESSION_OVERRIDE=sess-21
bash "$I" acquire shepherd-20 >/dev/null 2>&1
assert_eq "a refused acquire (live holder) logs REFUSE" "$(awk '$3 == "identity" {print $5}' "$E" | tail -1)" "REFUSE"
bash "$I" touch shepherd-20 >/dev/null 2>&1
assert_eq "a refused touch logs REFUSE" "$(awk '$3 == "identity-touch" {print $5}' "$E" | tail -1)" "REFUSE"
export SHEPHERD_LIVENESS_OVERRIDE="w6:p21:sess-21"
bash "$I" acquire shepherd-20 >/dev/null
assert_eq "a takeover of a gone id logs IDENTITY with the takeover detail" \
  "$(awk '$3 == "identity" {print $5, $6, $7}' "$E" | tail -1)" "IDENTITY took over"
printf '' > "$L/shepherd-22.lock"
bash "$I" acquire shepherd-22 >/dev/null 2>&1
assert_eq "a malformed identity lock logs MALFORMED" "$(awk '$3 == "identity" {print $4, $5}' "$E" | tail -1)" "shepherd-22 MALFORMED"
rm -f "$L/shepherd-22.lock"
assert_eq "every events line has at least five columns" "$(awk 'NF < 5 {bad++} END{print bad+0}' "$E")" "0"
# the shared id pattern still rejects what it always rejected
assert_fail "the shared pattern still rejects a malformed id" bash "$I" acquire not-a-shepherd
out=$(bash "$I" acquire not-a-shepherd 2>&1); rc=$?
assert_eq "and exits 2 naming the rule" "$rc $(printf '%s' "$out" | grep -c '^ERROR: bad shepherd id: ')" "2 1"
