...
Move the caret onto `1. Yes, I trust this folder` first (`send-keys <pane> Down`, or `Tab` on builds that cycle), **re-read the pane and confirm the caret is on that line**, and only then send `Enter`. Never confirm a selection you have not read back; `shepherd-smoke`'s `select_trust_yes` is the worked implementation. Expect the dialog on every first dispatch into a repo shepherd just created; it is a routine permission prompt under the manual §4, not an escalation.

## Where the rule stands

Adapter Gotchas (two lines); `shepherd-smoke select_trust_yes`.
