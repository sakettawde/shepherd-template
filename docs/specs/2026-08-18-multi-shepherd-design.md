...
Status: draft, pending Saket's approval
Supersedes: the "one shepherd at a time" invariant (CLAUDE.md §8, memory `one-shepherd-at-a-time`)

## 1. Goal

Let two or more shepherd Claude sessions run concurrently in the same
one instance clone, sharing one ledger, one registry and one git working
tree, without corrupting each other's state.

Today exactly one shepherd may hold the ledger. On 2026-07-29 two ran in parallel and
produced duplicate watchers on T-0019/T-0020, back-to-back "watchers re-armed" commits, and
two nudge prompts into one worker pane. The current rule is "detect a rival and stop".

### Non-goals

- Distributed operation. All instances share one filesystem and one machine.
- Automatic load balancing or task stealing. Reassignment happens only on Saket's word.
...
```bash
cd <instance-root> && SHEPHERD_ID=shepherd-1 claude -n shepherd-1 --remote-control shepherd-1
```
...
- The id must match `^shepherd-[a-z0-9]+(-[a-z0-9]+)*$` — the `shepherd-` prefix,
  ...
  `shepherd-collie` are equally valid. The prefix is required because §6.4's sweep
...
- **Create fails and the holder is live** (§3.3) → **refuse to start.** Report the collision to
  Saket and stop. Two instances sharing an id would each believe they own the other's tasks.
...
Verified directly against the herdr 0.7.4 binary on this machine:
...
with exit 0 — which was never checked against the real binary before Task 4b's implementation.
...
```json
{
  ...
  "started": "2026-08-18T14:02:00+05:30",
  "last_seen": "2026-08-18T15:31:00+05:30"
}
```
...
The id stays `T-NNNN`. Ownership lives in a card field, never in the filename, because Saket
wants reassignment: an owner-encoded id would either lie after a handover or force a rename
that breaks every reference in git history, decisions and memory.

### 4.1 `shepherd-reserve <shepherd-id> <pane> <session>`
...
Triage then fills the card from `${CLAUDE_PLUGIN_ROOT}/templates/task-card.md`, replacing the reservation line.

Verified empirically on this machine: 100 processes racing one filename yield exactly one
winner, five rounds out of five; 60 concurrent reservation loops yield 60 distinct ids.
...
ambiguous. Task 4b closed a narrower version of the same failure: collapsing "unresolved" into
"gone" let one unreachable pane free a live claimant's reservation exactly as the age-based timer
once did.
...
New field in `${CLAUDE_PLUGIN_ROOT}/templates/task-card.md`, directly under `state:`:
...
- it dispatches anyway, writes another instance's card, and arms watchers that the
  ...
  processing it, or both instances act. That is the 2026-07-29 incident, reintroduced by the
...
4. If it belongs to a **gone** instance → report it to Saket in one line as awaiting
...
`shepherd-lock acquire|takeover|release|check|sweep <name> [holder] [pane] [session] [task]`
...
`worker-cap` is a number in the CLAUDE.md Operator block, default **6**. This is a deliberate
policy change from the current "max 3 concurrent workers" (CLAUDE.md §2 rule 3), approved by
Saket on 2026-08-18.
...
under `dispatch.lock` the claiming instance writes **`pane: claiming-<shepherd-id>-T-NNNN`**
alongside `state: briefed`, and the count treats any `claiming-*` value as one occupied slot
exactly like a real pane id. Step 4 overwrites the placeholder with the real pane; an abort
before that reverts both fields.

The placeholder carries the **task id** as well as the instance id because the count is of
*distinct* values: one instance holding two simultaneous claims spelled alike would see them
collapse into a single occupied slot, and the cap would overshoot by one per extra claim. Lane
selection (dispatch precondition 5) makes back-to-back claims on two independent siblings the ordinary move rather
than a rarity, so the two claims have to be distinguishable.

The only race is two instances counting simultaneously and both dispatching. `dispatch.lock`
closes it: held for seconds across "count → decide → write `state: briefed` +
`pane: claiming-<shepherd-id>-T-NNNN`", then released. A loser of `dispatch.lock` never spins (§6.5): it
...
report it to Saket at session start, and leave the running worker untouched. It waits for
reassignment (§10).
...
for a long time — stalled on a permission prompt, blocked on Saket overnight, or compacted into
forgetting it holds anything. There is no clean automatic answer, so the spec states the limits
instead of pretending otherwise:

- A `card-*.lock` or `dispatch.lock` held by a **live** instance for more than 10 minutes is
  reported to Saket. It is never stolen. Saket decides.
...
- Clone id is `<slug>~<N>`, e.g. `karta~2`. The base working copy has no suffix.
- A task card's `project:` field carries the clone id: `project: karta~2`.
...
- The registry card gains a `## Clones` section:
  ...
  |---|---|---|---|
  | karta~2 | <code-dir>/workos/karta-wt2 | T-0091 | w6:p4 |

  No card carried this section when this was written and no template seeded it;
  `${CLAUDE_PLUGIN_ROOT}/templates/registry-card.md` seeds it since T-0215, and a card that lacks it gets it from
  dispatch step 0, under `card-<slug>`, the first time a `~N` task needs a working copy. From
  then on it is a permanent fixture of that card like the four below — listed in CLAUDE.md §5
  so a later normalising pass cannot drop it.
...
### 7.2 Creation and reuse

`shepherd-lane` is the procedure and dispatch step 0 is the call and its verdict
table; this section describes what the two do. A detached worktree never moves on its own, so
both paths below end with the working copy at the current dev-branch tip
(docs/incidents/2026-09-02-stale-clone-tip.md).

**The tip.** `git -C <parent-path> fetch origin` first. A configured `origin` whose fetch
fails is `JUDGE fetch failed`, a stop: every ref below it may be stale. A repo with **no**
`origin` is not that case and never was — it answers from the refs it has, under a `NOTE`.
Then `$tip` is the local `<dev-branch>` when it verifies and is strictly ahead of
`origin/<dev-branch>` (the self-repo, whose local `main` is routinely ahead),
`origin/<dev-branch>` when the remote is at or ahead of the local branch, and whichever of the
two exists otherwise (no `origin`, or no local branch yet). Local and remote each holding
commits the other lacks is `JUDGE diverged`: no tip is right, so dispatch undoes its claim and
reports to Saket rather than choosing for him.

**A `## Clones` row exists — reuse the worktree, reset it first.** The path has to be a
worktree of the parent, not merely a git repository: only the shared object store makes the
parent's tip mean anything there, so anything else is `HOLD lane path is not a worktree of
<parent>` rather than a reset that would detach a stranger's checkout. Then `git -C <path>
status --porcelain` non-empty is `HOLD dirty <path>`: the claim is undone and the path reported with
the first porcelain lines, never reset over — uncommitted work would ride onto a different
base, and the directory is Saket's call. A row whose path is gone is the no-row case after
`git -C <parent-path> worktree prune`, its row rewritten. Clean →
`git -C <path> checkout --detach "$tip"`.

**No row — create the working copy first**, and only where nothing is: a path that already
holds something is `HOLD lane path exists`, undone and reported, never reset or removed.

```bash
git -C <parent-path> worktree add --detach <parent-path>-wt<N> "$tip"
```
...
net against two workers on one branch. It is **not** one against two working copies at one
path: `worktree add --detach <existing-dir> <ref>` adopts the directory, empty or not, rather
than refusing it (git 2.43.0, measured 2026-09-08), which is why the script tests the path
itself before it creates anything.
...
`git worktree remove` runs only on Saket's word. A worktree may hold uncommitted work, and
...
`shepherd-commit "<message>" <path>...` runs:
...
- On give-up, report loudly and name the still-staged paths. Do not attempt an unstage — that
  takes the same lock.
- State commits never push. Pushing stays a manual act by Saket, or the framework backport to
  the `template` remote.

### 8.2 Decision log

Decisions move to `decisions/YYYY-MM-<shepherd-id>.md`. Retro and audits grep
`decisions/YYYY-MM*.md`, which also matches the legacy unsuffixed files. Existing
`decisions/2026-07.md` and `decisions/2026-08.md` stay where they are.
...
`shepherd-commit` exits 1. So it is locked and never committed. Its principal writers are retro
...
6. **Reconcile self:** every lock naming me must correspond to a card I own, and every active
   ...
   half-finished reassignment (§10). Report each mismatch to Saket; repair only the unambiguous
   direction (I own the card and no live instance holds the lock → take the lock, with
   `shepherd-lock takeover`, §6 — it re-checks liveness itself and refuses if the holder turns out live
...
Triggered only by Saket: *"shepherd-2, take T-0091."*

Order matters. The card is written **before** the lock, so that every crash point leaves a state
that §9 step 6 can detect:

1. Confirm the previous owner is not live, or that Saket has said to take it anyway.
...
3. Take `project-<clone-id>.lock`:
   ...
   - Held by a gone instance under the orphan rule → `shepherd-lock takeover` (§6), which does the
...
| memory `one-shepherd-at-a-time` | rewritten as the multi-instance contract |
...
1. **Reserve under contention** — 30 parallel `shepherd-reserve` → 30 distinct ids, every file
...
3. **Simultaneous same-id boot** — two sessions, same `SHEPHERD_ID`. Exactly one starts; the other
   refuses and reports.
4. **Commit under contention** — 5 parallel `shepherd-commit` on distinct paths → 5 commits, no
...
- **Live-but-wedged holders** is unresolved automatically (§6.5). They are reported to Saket.
- **Orphaned workers.** A gone shepherd's worker keeps running with nobody watching. §6.4 makes
  this visible instead of silent, but it still needs Saket to reassign.
...
- **Legacy cards** default to `owner: shepherd-1`. If shepherd-1 is not running, those cards get
  no watchers and — holding no locks — no orphan report either. There are no active cards at the
  time of writing, so this is a transition note, not a live problem.
- **Filesystem assumption.** O_EXCL and atomic rename are verified on this WSL2 ext4 volume. If
  the repo ever moves under `/mnt/*` (9p/drvfs), re-verify before trusting either.

## 14. Review record

v1 was reviewed adversarially by a fresh Fable session with instructions to break it, including
...
- On commits it noted the retry budget failing a 20-way parallel drill. The budget is raised, but
  ...
  failure mode (a stale shared index showing phantom modifications to Saket) is worse than the
  problem.
